...
def create_app() -> Flask:
    ...
    if settings.api.rate_limit_enabled:
        ...
        logger.info(
            f"Rate limiting enabled: {settings.api.rate_limit_per_minute} requests/minute"
        )
    ...
    swagger_template = {
        "info": {
            ...
            "contact": {"name": "Gauransh Gupta", "email": "gauransh@northeastern.edu"},
        },
        "schemes": ["http", "https"],
        "tags": [
            {
    ...
    logger.info(
        f"API documentation: http://{settings.api.host}:{settings.api.port}/docs"
    )
    logger.info("=" * 60)
