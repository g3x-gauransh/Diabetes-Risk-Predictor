...
def create_feature_importance_chart(patient_data: Dict[str, Any]) -> go.Figure:
    ...
    colors = [
        (
            "red"
            if s in ["High", "Diabetic", "Obese", "Higher Risk"]
            else "orange" if s in ["Prediabetic", "Overweight", "Low"] else "green"
        )
        for s in statuses
    ]
    ...
    fig.update_layout(
        title="Key Health Indicators", yaxis_title="Value", height=400, showlegend=False
    )
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">🏥 Diabetes Risk Predictor</h1>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<p class="sub-header">AI-Powered Diabetes Risk Assessment System</p>',
        unsafe_allow_html=True,
    )
    ...
    if page == "🔮 Make Prediction":
        ...
        with col2:
            predict_button = st.button(
                "🔮 Predict Diabetes Risk", use_container_width=True, type="primary"
            )
                        ...
                        st.json(
                            {
                                ...
                                "risk_level": result["risk_level"],
                                "request_duration_ms": result.get(
                                    "request_duration_ms", "N/A"
                                ),
                            }
                        )
    ...
    # ========================================================================
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                with col2:
                    st.metric(
                        "Architecture", model_info.get("model_architecture", "N/A")
                    )
                with col3:
                    st.metric(
                        "Input Features", len(model_info.get("input_features", []))
                    )
                ...
                if metrics:
                    ...
                    with col1:
                        ...
                        st.metric("Training AUC", f"{metrics.get('final_auc', 0):.4f}")
                    ...
                    with col2:
                        ...
                        st.metric(
                            "Validation AUC", f"{metrics.get('final_val_auc', 0):.4f}"
                        )
