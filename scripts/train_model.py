...
class ModelTrainer:
    ...
    def validate_model_predictions(
        ...
        logger.info(f"    Low risk (<30%):     {low_risk} ({low_risk/len(y_pred_proba)*100:.1f}%)")
        logger.info(f"    Medium risk (30-70%): {medium_risk} ({medium_risk/len(y_pred_proba)*100:.1f}%)")
        logger.info(f"    High risk (>70%):    {high_risk} ({high_risk/len(y_pred_proba)*100:.1f}%)")
    ...
    def run_training_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.model.build_model(
                ...
                hidden_layers=self.config.get('hidden_layers', settings.model.hidden_layers),
                dropout_rate=self.config.get('dropout_rate', settings.model.dropout_rate),
                learning_rate=self.config.get('learning_rate', settings.model.learning_rate)
            )
    ...
    def _save_training_report(self) -> None:
        ...
        # Save report
        report_path = settings.model.model_dir / f'training_report_{settings.model.version}.json'
    ...
    def _print_training_summary(self) -> None:
        ...
        if training:
            ...
            logger.info(f"  Final Training Accuracy: {training.get('final_accuracy', 0):.4f}")
            logger.info(f"  Final Training AUC: {training.get('final_auc', 0):.4f}")
            
            if 'final_val_loss' in training:
                logger.info(f"  Final Validation Loss: {training.get('final_val_loss', 0):.4f}")
                logger.info(f"  Final Validation Accuracy: {training.get('final_val_accuracy', 0):.4f}")
                logger.info(f"  Final Validation AUC: {training.get('final_val_auc', 0):.4f}")
        
        # Evaluation metrics
        logger.info("\n📈 TEST SET PERFORMANCE:")
        evaluation = self.results.get('evaluation', {})
        logger.info(f"  Accuracy:  {evaluation.get('accuracy', 0):.4f} ({evaluation.get('accuracy', 0)*100:.2f}%)")
        ...
        logger.info(f"  Confusion Matrix: {settings.model.model_dir / 'confusion_matrix.png'}")
        logger.info(f"  Training History: {settings.model.model_dir / 'training_history.png'}")
        logger.info(f"  Training Report: {settings.model.model_dir / f'training_report_{settings.model.version}.json'}")
...
def parse_arguments():
    ...
    parser.add_argument(
        '--force-save',
        action='store_true',
        help='Save model even if validation fails'
    )
