...
class DataPreprocessor:
    ...
    def validate_data(self, df: pd.DataFrame) -> None:
        ...
        missing_cols = set(self.feature_columns + [self.target_column]) - set(
            df.columns
        )
        if missing_cols:
            raise DataValidationError(
                f"Missing required columns: {missing_cols}", field=None
            )
        ...
        for col in self.feature_columns:
            if not np.issubdtype(df[col].dtype, np.number):
                raise DataValidationError(
                    f"Column {col} must be numeric, got {df[col].dtype}", field=col
                )
    ...
    def handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in zero_not_possible:
            if col in df.columns:
                ...
                if zero_count > 0:
                    logger.info(
                        f"Found {zero_count} zeros in {col}, replacing with median"
                    )
    ...
    def split_data(
        ...
        logger.info(
            f"Class distribution in training: {y_train.value_counts().to_dict()}"
        )
