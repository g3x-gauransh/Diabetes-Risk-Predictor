...
class PredictionService:
        ...
        try:
            ...
            if not settings.model.scaler_path.exists():
                raise FileNotFoundError(
                    f"Scaler not found: {settings.model.scaler_path}"
                )
    ...
    def predict_batch(self, patients: List[PatientData]) -> List[PredictionResponse]:
        ...
        try:
            ...
            probabilities = self.model.predict(
                features_scaled, return_proba=True
            ).flatten()
    ...
    def _generate_recommendations(
        self, patient: PatientData, prediction: int, probability: float
    ) -> List[str]:
        """
        ...
        if patient.glucose > 140:
            recommendations.append(
                "High glucose detected - dietary modifications recommended"
            )
        ...
        if patient.bmi > 30:
            ...
            recommendations.append(
                "BMI indicates overweight - consider lifestyle modifications"
            )

        # Age-specific
        if patient.age > 45:
            recommendations.append(
                "Regular health screenings recommended for your age group"
            )
