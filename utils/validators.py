...
class PatientData(BaseModel):
    ...
    glucose: float = Field(
        ..., ge=0, le=300, description="Plasma glucose concentration"
    )
    ...
    bmi: float = Field(
        ..., ge=0, le=70, description="Body mass index (weight in kg/(height in m)^2)"
    )
    diabetes_pedigree_function: float = Field(
        ..., ge=0, le=2.5, description="Diabetes pedigree function"
    )
...
class PredictionResponse(BaseModel):
    ...
    prediction: int = Field(
        ..., ge=0, le=1, description="0 = No Diabetes, 1 = Diabetes"
    )
...
def validate_patient_data(data: Dict[str, Any]) -> PatientData:
    ...
    try:
        ...
        raise DataValidationError(
            message=f"Invalid patient data: {'; '.join(errors)}", field=None
        )
